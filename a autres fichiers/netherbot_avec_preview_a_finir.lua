...
--  Net-helper
--------------------------------------------------------------------
local NB_DATA = _G.NB_DATA or error("NB_DATA manquant")

local function dbg(...) print("|cff55ff55[NB-DBG]|r", ...) end
local function list(tbl) local t={}; for k,v in pairs(tbl) do t[#t+1]=k end; return #t end
...
function NetherBot:ShowCreateBotFrame()
  ...
  local f   = GUI:Create("Frame"); f:SetTitle("Création NPC-Bot"); f:SetLayout("Flow"); f:SetWidth(660); f:SetHeight(540)
  self.createBotFrame = f

  local function tip(wdg, txt)
    local tgt = wdg.editbox or wdg.frame
    tgt:EnableMouse(true)
    tgt:SetScript("OnEnter", function(s) GameTooltip:SetOwner(s,"ANCHOR_RIGHT"); GameTooltip:AddLine(txt,1,1,1,true); GameTooltip:Show() end)
    tgt:SetScript("OnLeave", GameTooltip_Hide)
  end

  -- champs texte/dropdown
  local nameBox = GUI:Create("EditBox"); nameBox:SetLabel("Nom"); nameBox:SetFullWidth(true); tip(nameBox,"Première lettre Maj."); f:AddChild(nameBox)

  local ddClass  = GUI:Create("Dropdown"); ddClass:SetLabel("Classe");  ddClass:SetList(NB_DATA.Classes);  ddClass:SetWidth(200); f:AddChild(ddClass)
  local ddRace   = GUI:Create("Dropdown"); ddRace :SetLabel("Race");    ddRace :SetList(NB_DATA.Races);    ddRace :SetWidth(200); f:AddChild(ddRace)
  local ddGender = GUI:Create("Dropdown"); ddGender:SetLabel("Genre");  ddGender:SetList(NB_DATA.Genders); ddGender:SetWidth(120); f:AddChild(ddGender)

  local ddSkin   = GUI:Create("Dropdown"); ddSkin:SetLabel("Peau");     ddSkin:SetWidth(90);  f:AddChild(ddSkin)
  local ddFace   = GUI:Create("Dropdown"); ddFace:SetLabel("Visage");   ddFace:SetWidth(90);  f:AddChild(ddFace)
  local ddHair   = GUI:Create("Dropdown"); ddHair:SetLabel("Coiffure"); ddHair:SetWidth(110); f:AddChild(ddHair)
  local ddHairC  = GUI:Create("Dropdown"); ddHairC:SetLabel("Couleur"); ddHairC:SetWidth(110);f:AddChild(ddHairC)
  local ddFeat   = GUI:Create("Dropdown"); ddFeat:SetLabel("Ornements");ddFeat:SetWidth(90);  f:AddChild(ddFeat)
  local ddSS     = GUI:Create("Dropdown"); ddSS  :SetLabel("Voix");     ddSS:SetList(NB_DATA.Soundset); ddSS:SetWidth(90); f:AddChild(ddSS)

  ------------------------------------------------------------------
  --  Modèle 3D (Model + SetDisplayInfo)
  ------------------------------------------------------------------
local preview = CreateFrame("DressUpModel", nil, f.frame)
preview:SetSize(240, 330)
preview:SetPoint("TOPRIGHT", -25, -60)
preview:ClearModel()
preview:SetRotation(math.pi * 1.4)      -- fonctionne sur DressUpModel
-- === DEBUG : lister toutes les méthodes disponibles sur le modèle ===
do
  local meta = getmetatable(preview)
  if meta and meta.__index then
    dbg("---- METHODS ON PREVIEW ----")
    for k in pairs(meta.__index) do
      dbg("  *", k)
    end
    dbg("-----------------------------")
  else
    dbg("!!! getmetatable(preview) sans __index")
  end
end
-- ====================================================================

dbg("DisplayID table lookup:", NB_DATA.DisplayID[ddRace:GetValue()])

 local function applyDisplay(path)
  if not path then dbg("applyDisplay path NIL") return end
  preview:ClearModel()
  preview:SetModel(path)
  preview:SetFacing(math.pi*1.4)
  dbg("   + model", path, "chargé")
end

  ------------------------------------------------------------------
  --  Ranges Peau/Visage etc.
  ------------------------------------------------------------------
  local function fillRanges()
    local r,g = ddRace:GetValue(), ddGender:GetValue()
    dbg("fillRanges r",r,"g",g)
    if not (r and g) then return end

    local vis = NB_DATA:BuildRangeList(r,g,"skin")
    dbg("   skin entries", list(vis))
    ddSkin   :SetList(vis)
...
local function refreshPreview()
  local r,g = ddRace:GetValue(), ddGender:GetValue()
  dbg("refreshPreview r",r,"g",g)
  if not (r and g) then return end
  local path = NB_DATA.ModelPath[r] and NB_DATA.ModelPath[r][g]
  dbg("   path =", path)
  applyDisplay(path)
end

  local function changed(w,e,val)
    dbg("changed", w.type, val)
    fillRanges(); refreshPreview()
  end

  -- bindings
  ddRace  :SetCallback("OnValueChanged", changed)
  ddGender:SetCallback("OnValueChanged", changed)
  ...
  --  Bouton créer
  ------------------------------------------------------------------
  local b = GUI:Create("Button"); b:SetText("Créer"); b:SetWidth(130); f:AddChild(b)
  b:SetCallback("OnClick", function()
    local name=(nameBox:GetText() or ""):gsub("%s+","_")
    local class=ddClass:GetValue();local race,gen=ddRace:GetValue(),ddGender:GetValue()
    local skin,face,hair,col,feat=ddSkin:GetValue(),ddFace:GetValue(),ddHair:GetValue(),ddHairC:GetValue(),ddFeat:GetValue()
    local ss=ddSS:GetValue()
    if name=="" or not(class and race and gen and skin and face and hair and col and feat and ss) then
      print("|cffff0000[NB]|r Champs manquants."); return end
    local cmd=string.format(".npcbot createnew %s %d %d %d %d %d %d %d %d %d",
      name,class,race,gen,skin,face,hair,col,feat,ss)
    dbg("CMD:",cmd); SendChatMessage(cmd,"SAY"); f:Hide()
  end)
end
